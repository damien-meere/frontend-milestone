...
This project website will highlight my capability to employ the various tools illustrated throughout the User-centric frontend development section of the Code Institute programme. 
...
within its own page. The dropdown menus within the navigation bar link to various sections within these strands. There is also a contact page with details of the company and a form with which to harness
feedback from users.
...
## UX+User-Stories
My goal in developing this site was to make it easy for training candidates to access information on available training options, through a minimalist, sleek, easy-to-navigate site. Therefore, in designing
this site, I started with a number of user stories:
...
From these user-stories it was clear that quick and easy access to information would be key to ensuring that users choose 'Eolas' as their training provider. I needed to make sure that users could quickly
...
required. Also within this wireframe folder is a number of designs for both the navbar (full and collapsed) and footer, as well as designs for the course booking modals so prevalent throughout the site.
...
On larger screens, more verbose content is presented. However, on smaller screens, text is kept to a minimum, so as to ensure only the most pertinent information is presented. Every effort has been
made to ensure that this doesn't negatively affect the user's utilisation of the site, and that all requisite information is still effectively conveyed to the site visitor. In some cases, the alteration
...
proper user data is harnessed. Proper formatting is forced on Email addresses, and where required input to a message text area is enforced (see contact page and 'Get in Touch' button)
...
The Technical page is broken into five separate sections, each of which fills the full screen. The first is a welcome panel describing the company's technical training services. The second is illustrates
the available Cisco programmes (CCNA, CCNP, CCIE). The buttons each link to a booking form modal (as illustrated in the General Notes section). Some programmes are a composite of multiple training courses;
therefore there is a dropdown menu for users to book the requisite course. The third and fourth sections are similar in structure to this, but cover Nokia and Cybersecurity offerings. The final section
...
The Professional page is similar in structure to the Technical page, except this page only has three sections, a welcome panel and two training panels covering Management training and Organisational
change training.
...
Similar to the Professional and Technical pages, this page is broken into a Welcome panel and a training panel. Within the training panel, a number of available training options are highlighted
all related to soft-skills/personal development.
...
The Contact page contains two sections, the first of which calls out the location information and contact details for Eolas. The second panel presents the user with a Contact form. This form has some 
...
in the Navbar (visible on larger screens).There is currently no back-end to harness the user data, so this is merely in place to demonstrate this functionality.
...
## Future-Features 
In future, the booking form illustrating course availability progress bars, will be linked to a working database in order to give training candidates an accurate view of available space on the course. 

At the moment, user inputs are acknowledged through automated response modals. In the next iteration of this site, user inputs will be harnessed from the booking forms and contact forms, and actioned within
...
form for them to complete to request a space on the course. On completion of this form, the user is presented with a modal calling out that a training rep will be in contact within 24 hours. The plan for 
...
page with the modal overlaid as illustrated [HERE](https://damien-meere.github.io/frontend-milestone/contact.html#submissionModal). The request response modal on submission of a course request modal
...
page, so as to allow users to quickly navigate to the requisite area on the site. 
...
of section heads for both large screens and small. When the site width is being reduced, once a certain threshold is met, the headers will shift. The smaller headers are better suited to the small screens,
...
### Content
Most of the content on the site was written by me. However, there are course details & descriptions on the site that apply to real-world certifications and accreditations. As such, in some cases, 
the content on the site is an adapted version of these descriptions.
